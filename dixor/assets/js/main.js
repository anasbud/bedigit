...
(function($) {
	...
	$(document).ready(function() {
        ...
        // Select
        // 1) Récupérer les titres des projets depuis les blocs (compat vieux navigateurs)
        var titles = [];
        ...
        // 2) Cibler les selects du formulaire (sans changer ton HTML)
        var $form = $('.contact-form');
        var $selects = $form.find('#choice1, #choice2, #choice3');

        // 3) Remplir chaque select en conservant le placeholder existant (1ère option)
        ...
        // 4) Empêcher les doublons entre selects (sans Set)
        function syncOptions() {
            // Récupérer les valeurs choisies (non vides)
            var chosen = {};
            $selects.each(function () {
                var v = $(this).val();
            ...
            // Pour chaque select, désactiver les options choisies ailleurs
            $selects.each(function () {
                var currentVal = $(this).val();
                $(this).find('option').each(function () {
                    var val = $(this).val();
                    if (!val) return; // ignorer placeholder
                    // Désactive si sélectionné ailleurs ET pas la valeur actuelle de ce select
                    var disable = (chosen[val] && val !== currentVal) ? true : false;
                    // .prop gère bien la désactivation même sur anciens navigateurs
